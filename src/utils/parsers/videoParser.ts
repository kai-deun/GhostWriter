...
export async function parseVideoMetadata(file: File): Promise<MetadataInfo> {
  ...
  if (!result.success) {
    throw new Error(`Failed to parse video metadata: ${result.error}`);
  }
...
export async function writeVideoMetadata(file: File, updatedTags: ExifTag[], scrubAll: boolean): Promise<{ url: string, warnings: string[] }> {
  const name = file.name.toLowerCase();
  
  if (name.endsWith('.mkv') || name.endsWith('.webm')) {
    throw new Error(`${file.name} uses the Matroska (MKV/WebM) container, which has write restrictions in a browser-only environment. Writing or stripping metadata for Matroska files is read-only here, as saving changes requires native utilities like mkvpropedit.`);
  }

  if (name.endsWith('.avi')) {
    throw new Error(`${file.name} is an AVI file (RIFF format), which has write restrictions in this application. Saving or stripping metadata for AVI files is not supported by ExifTool.`);
  }
  ...
  if (!result || !result.success) {
    if (undefinedTags.length > 0) {
      throw new Error(`Failed to process video metadata: ${result?.error}. (Also could not modify: ${undefinedTags.join(", ")})`);
    }
    throw new Error(`Failed to process video metadata: ${result?.error}`);
  }
